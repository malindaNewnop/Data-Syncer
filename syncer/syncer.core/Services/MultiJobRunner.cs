            ...
            IFileEnumerator fileEnumerator)
        {
            ...
            try
            {
                ...
                if (config != null)
                {
                    ...
                    _logService.LogInfo($"MultiJobRunner using configuration-based concurrent limit: {_maxConcurrentJobs}", "MultiJobRunner");
                }
            }
            catch (Exception ex)
            {
                _logService.LogWarning($"Failed to load MultiJobConfiguration, using default limit: {ex.Message}", "MultiJobRunner");
            }
