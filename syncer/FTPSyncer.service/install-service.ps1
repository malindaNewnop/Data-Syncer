...
function Find-ServiceExecutable {
    ...
    $possiblePaths = @(
        Join-Path $scriptPath "bin\Release\FTPSyncer.service.exe"
        Join-Path $scriptPath "bin\Debug\FTPSyncer.service.exe"
    )
...
function Install-FTPSyncerService {
    ...
    if (-not $exePath) {
        ...
        Write-Host "2. Right-click 'FTPSyncer.service' project -> Build" -ForegroundColor Yellow
        return $false
    }
