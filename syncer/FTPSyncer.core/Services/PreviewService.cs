    ...
    public class PreviewService : IPreviewService
    {
        ...
        public bool ValidateJob(SyncJob job, out List<string> validationErrors)
        {
            ...
            try
            {
                ...
                if (FTPSyncer.core.StringExtensions.IsNullOrWhiteSpace(job.Name))
                {
                    validationErrors.Add("Job name is required");
                }

                if (FTPSyncer.core.StringExtensions.IsNullOrWhiteSpace(job.SourcePath))
                {
                    validationErrors.Add("Source path is required");
                }

                if (FTPSyncer.core.StringExtensions.IsNullOrWhiteSpace(job.DestinationPath))
                {
                ...
                {
                    if (FTPSyncer.core.StringExtensions.IsNullOrWhiteSpace(job.Schedule.CronExpression))
                    {
