...
namespace FTPSyncer.ui.Services
{
    ...
    public class TimerJobManager : ITimerJobManager
    {
        ...
        private class TimerJobInfo
        {
            ...
            public FTPSyncer.core.ConnectionSettings ConnectionSettings { get; set; }
        ...
        public bool RegisterTimerJob(long jobId, string jobName, string folderPath, string remotePath, double intervalMs, bool includeSubfolders, bool deleteSourceAfterTransfer, bool enableFilters, List<string> includeExtensions, List<string> excludeExtensions)
        {
            try
            {
                ...
                var coreSettings = new FTPSyncer.core.ConnectionSettings
                {
                    Protocol = connectionSettings.Protocol == "SFTP" ?
                        FTPSyncer.core.ProtocolType.Sftp :
                        connectionSettings.Protocol == "FTP" ?
                            FTPSyncer.core.ProtocolType.Ftp :
                            FTPSyncer.core.ProtocolType.Local,
                ...
                try
                {
                    var bandwidthService = FTPSyncer.core.Services.BandwidthControlService.Instance;
                    if (bandwidthService.IsBandwidthControlEnabled && coreSettings.Protocol == FTPSyncer.core.ProtocolType.Sftp)
                    {
                        // For upload jobs, apply upload bandwidth limits
                        var sftpConfig = new FTPSyncer.core.Configuration.SftpConfiguration();
                ...
                FTPSyncer.core.TransferClientFactory factory = new FTPSyncer.core.TransferClientFactory();
        ...
        private bool RestoreTimerJob(PersistentTimerJob persistentJob)
        {
            try
            {
                ...
                FTPSyncer.core.ProtocolType coreProtocol = FTPSyncer.core.ProtocolType.Ftp;
                switch (connectionSettings.Protocol.ToUpper())
                {
                    case "SFTP":
                        coreProtocol = FTPSyncer.core.ProtocolType.Sftp;
                        break;
                    case "FTP":
                        coreProtocol = FTPSyncer.core.ProtocolType.Ftp;
                        break;
                    case "LOCAL":
                        coreProtocol = FTPSyncer.core.ProtocolType.Local;
                        break;
                    default:
                        coreProtocol = FTPSyncer.core.ProtocolType.Ftp;
                        break;
